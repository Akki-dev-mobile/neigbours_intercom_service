...
enum IntercomRefreshFailureType {
  temporary,
  invalidGrant,
  unauthorized,
  unknown,
...
@immutable
class IntercomTokenRefreshResult {
  final bool refreshed;
  final IntercomAuthTokens? tokens;
  final IntercomRefreshFailureType? failureType;
  final String? message;

  const IntercomTokenRefreshResult._({
  ...
  const IntercomTokenRefreshResult.success({
    required IntercomAuthTokens tokens,
    String? message,
  }) : this._(refreshed: true, tokens: tokens, message: message);

  const IntercomTokenRefreshResult.failure({
    required IntercomRefreshFailureType failureType,
    String? message,
  }) : this._(refreshed: false, failureType: failureType, message: message);

  const IntercomTokenRefreshResult.notSupported()
    : this._(refreshed: false, failureType: IntercomRefreshFailureType.unknown);
}
...
abstract class IntercomAuthPort {
  ...
  /// Optional non-breaking hook: host app can refresh tokens.
  /// Default keeps legacy behavior.
  Future<IntercomTokenRefreshResult> refreshSession({
    bool force = false,
  }) async {
    return const IntercomTokenRefreshResult.notSupported();
  }

  /// Optional non-breaking hook: host app can perform logout/session-expired UX.
...
class IntercomEndpoints {
  ...
  static const IntercomEndpoints cubeOne = IntercomEndpoints(
    ...
    roomServiceBaseUrl: 'https://apigw.cubeone.in/chatapp/api/v1',
    callServiceBaseUrl: 'https://apigw.cubeone.in/meet-service/api/v1',
