...
class _GroupChatScreenState extends ConsumerState<GroupChatScreen>
  ...
  Future<void> _markMessageAsRead(String messageId) async {
    ...
    if (_readReceiptInFlight.contains(messageId) ||
      ...
      log(
        'ℹ️ [GroupChatScreen] Skipping duplicate read receipt for: $messageId',
      );
      return;
    }
    ...
    try {
      ...
      if (response.success) {
        ...
        log(
          '⚠️ [GroupChatScreen] Failed to mark message as read: ${response.error}',
        );
  ...
  void _setupMessageSync() {
    ...
    _messageSyncTimer = Timer.periodic(const Duration(seconds: 60), (
      ...
      if (_isWebSocketConnected) {
        log(
          '⏭️ [GroupChatScreen] Skipping message sync - WebSocket is connected',
        );
        return;
      }
      ...
      try {
        ...
        if (response.success && response.data != null && mounted) {
          ...
          log(
            '⚠️ [GroupChatScreen] Rate limit in message sync, pausing sync timer',
          );
  ...
  Future<void> _openRoom({
    ...
    if (_lastApiCallTime != null) {
      ...
      if (timeSinceLastCall < _minApiCallInterval) {
        log(
          '⏸️ [GroupChatScreen] Rate limiting: ${_minApiCallInterval.inSeconds - timeSinceLastCall.inSeconds}s remaining before next API call',
        );
    ...
    try {
      ...
      UnreadCountManager.instance.clearUnreadCount(widget.group.id).catchError((
        e,
      ) {
  ...
  Future<void> _handleWebSocketMessage(WebSocketMessage wsMessage) async {
    ...
    if (!_isUserMember || _hasLeftGroup) {
      log(
        '🚫 [GroupChatScreen] Ignoring WebSocket message - user is not a member or has left group',
      );
      return;
    }
    ...
    if (wsMessage.data?['type']?.toString().toLowerCase() == 'typing' ||
      ...
      final typingUserName =
          wsMessage.data?['user_name']?.toString() ??
    ...
    if (messageType == 'unread_count_update' ||
      ...
      if (roomId != null && userId != null && unreadCount != null) {
        log(
          '📊 [GroupChatScreen] Received unread_count_update: room=$roomId, user=$userId, count=$unreadCount',
        );
        ...
        if (userId == widget.currentUserId) {
          if (unreadCount == 0) {
            ...
            log(
              '📊 [GroupChatScreen] Unread count updated to $unreadCount for room $roomId',
            );
          }
        }
      }
    ...
    if (messageType == 'read_receipt' ||
      ...
      log(
        '📖 [GroupChatScreen] Read receipt received: message=$messageId, user=$userId, readAt=$readAt',
      );
      ...
      if (messageId != null && mounted) {
        ...
        setState(() {
          ...
          if (index != -1 &&
            ...
            log(
              '✅ [GroupChatScreen] Updated message to SEEN status: $messageId',
            );
          }
        });
      }
    ...
    if (messageType == 'delivered_receipt' ||
      ...
      log(
        '🚚 [GroupChatScreen] Delivered receipt received: message=$messageId, user=$userId',
      );
        ...
        setState(() {
          ...
          if (index != -1 &&
            ...
            if (_messages[index].status == GroupMessageStatus.sent ||
              ...
              log(
                '✅ [GroupChatScreen] Updated message to DELIVERED status: $messageId',
              );
            }
          }
        });
    ...
    if (messageType == 'presence_update' ||
      ...
      log(
        '👤 [GroupChatScreen] Presence update: user=$userId, online=$isOnline, status=$statusString',
      );
    ...
    if (wsMessage.messageTypeEnum == WebSocketMessageType.error) {
      ...
      if (isMembershipError) {
        log(
          '🚫 [GroupChatScreen] Membership error detected - removing optimistic messages and setting non-member status',
        );
        ...
        if (mounted) {
          setState(() {
            ...
            for (final msg in optimisticMessages) {
              ...
              log(
                '🗑️ [GroupChatScreen] Removed optimistic message: ${msg.id}',
              );
            }
          });
        }
    ...
    if (wsMessage.data != null) {
      ...
      log(
        '📥 [GroupChatScreen] WebSocket message received - content length: ${content.length}, data keys: ${wsMessage.data?.keys.toList() ?? []}',
      );
      ...
      if (roomMessage.replyTo != null && roomMessage.replyTo!.isNotEmpty) {
        ...
        replyToMessage = _findMessageById(roomMessage.replyTo, {}, _messages);
        ...
        if (replyToMessage != null) {
          ...
          log(
            '⚠️ [GroupChatScreen] Reply target message ${roomMessage.replyTo} not found in loaded messages (WebSocket). Using placeholder.',
          );
        }
      }
      ...
      final groupMessage = _convertRoomMessageToGroupMessage(
        roomMessage,
      ).copyWith(replyTo: replyToMessage);
        ...
        if (isFromCurrentUser) {
          ...
          if (mIsFromCurrentUser &&
            ...
            final timeDiff = m.timestamp
                .difference(groupMessage.timestamp)
                .inSeconds
                .abs();
            if (timeDiff < 10) {
              ...
              if (wsFileUrl != null && wsFileUrl.isNotEmpty) {
                ...
                if (m.firstImageUrl != null && wsFileUrl == m.firstImageUrl) {
                  log(
                    '✅ [GroupChatScreen] Matched optimistic message by image URL: $wsFileUrl',
                  );
                  return true;
                }
                ...
                if (m.documentUrl != null && wsFileUrl == m.documentUrl) {
                  log(
                    '✅ [GroupChatScreen] Matched optimistic message by document URL: $wsFileUrl',
                  );
                  return true;
                }
                ...
                if (m.videoUrl != null && wsFileUrl == m.videoUrl) {
                  log(
                    '✅ [GroupChatScreen] Matched optimistic message by video URL: $wsFileUrl',
                  );
                  return true;
                }
              }
              ...
              if (groupMessage.firstImageUrl != null ||
                ...
                if (groupMessage.firstImageUrl != null &&
                  ...
                  log(
                    '✅ [GroupChatScreen] Matched optimistic message by image URL (from groupMessage): ${groupMessage.firstImageUrl}',
                  );
                  return true;
                }
                // Match by document URL
                if (groupMessage.documentUrl != null &&
                  ...
                  log(
                    '✅ [GroupChatScreen] Matched optimistic message by document URL (from groupMessage): ${groupMessage.documentUrl}',
                  );
                  return true;
                }
                ...
                if (groupMessage.videoUrl != null &&
                  ...
                  log(
                    '✅ [GroupChatScreen] Matched optimistic message by video URL (from groupMessage): ${groupMessage.videoUrl}',
                  );
                  return true;
                }
              }
              ...
              if (wsFileUrl == null &&
                ...
                if (groupMessage.text.isNotEmpty &&
                  ...
                  log(
                    '✅ [GroupChatScreen] Matched optimistic message by text content',
                  );
                  return true;
                }
              }
              ...
              if (timeDiff < 3) {
                log(
                  '✅ [GroupChatScreen] Matched optimistic message by timestamp (fallback): $timeDiff seconds',
                );
                return true;
              }
            }
      ...
      if (existingIndex == -1) {
        ...
        if (_hasLeftGroup || !_isUserMember) {
          log(
            '🚫 [GroupChatScreen] Ignoring WebSocket message - user has left group',
          );
          return;
        }
        ...
        if (mounted) {
          setState(() {
            ...
            final insertIndex = _messages.indexWhere(
              (m) => m.timestamp.isAfter(groupMessage.timestamp),
            );
            if (insertIndex == -1) {
          ...
          if (roomMessage.eventType?.toLowerCase() == 'user_left') {
            ...
            log(
              '🚪 [GroupChatScreen] User left group - invalidated member cache for room: ${widget.group.id}',
            );
          }
        }
      } else {
        ...
        if (existingIndex < 0 || existingIndex >= _messages.length) {
          log(
            '⚠️ [GroupChatScreen] Invalid existingIndex: $existingIndex (messages.length: ${_messages.length}), skipping update',
          );
          return;
        }
        ...
        if (_hasLeftGroup || !_isUserMember) {
          log(
            '🚫 [GroupChatScreen] Ignoring message update - user has left group',
          );
          return;
        }
        ...
        if (mounted) {
          setState(() {
            ...
            if (existingIndex >= 0 && existingIndex < _messages.length) {
              ...
              log(
                '⚠️ [GroupChatScreen] Cannot update message - index out of bounds: $existingIndex (messages.length: ${_messages.length})',
              );
            }
            ...
            if (roomMessage.eventType?.toLowerCase() == 'user_left') {
              ...
              roomInfoCache.markMemberLeft(widget.group.id);
              log(
                '🚪 [GroupChatScreen] User left group (message update) - invalidated member cache for room: ${widget.group.id}',
              );
            }
          });
        }
  ...
  Future<void> _loadOlderMessages() async {
    ...
    try {
      ...
      final initialLimit = _currentOffset == 0
          ? _messagesPerPage * 2
          : _messagesPerPage;
  ...
  void _handleMessageError(int? statusCode, String errorMessage) {
    ...
    if (statusCode == 401) {
      ...
      log(
        '⚠️ [GroupChatScreen] Rate limit error (429). Retry count: $_rateLimitRetryCount, Backoff: ${backoffSeconds}s',
      );
  ...
  Future<void> _fetchMessagesDirectly() async {
    try {
      ...
      if (messagesResponse.success && messagesResponse.data != null) {
        final roomMessages = messagesResponse.data!;
        log(
          '✅ [GroupChatScreen] Successfully fetched ${roomMessages.length} messages directly',
        );
        ...
        if (widget.group.id.isNotEmpty && widget.group.id.contains('-')) {
          log(
            '🔌 [GroupChatScreen] Connecting WebSocket with UUID: ${widget.group.id}',
          );
        ...
        log(
          '❌ [GroupChatScreen] Failed to fetch messages directly: ${messagesResponse.error}',
        );
        // If even direct fetch fails, show error
        _handleMessageError(
          messagesResponse.statusCode,
          messagesResponse.displayError,
        );
      }
    } catch (e) {
  ...
  GroupMessage _convertRoomMessageToGroupMessage(
    ...
    final senderId = isFromCurrentUser
        ? widget.currentUserId
        : roomMessage.senderId;
    ...
    if (senderAvatar == null || senderAvatar.isEmpty) {
      ...
      senderAvatar = _getAvatarNormalized(
        ...
        numericId:
            roomMessage.snapshotUserId?.toString() ??
    ...
    log(
      '🔍 [GroupChatScreen] Converting RoomMessage - messageType: ${roomMessage.messageType}, body length: ${roomMessage.body.length}',
    );
    log(
      '   Body preview: ${roomMessage.body.length > 100 ? roomMessage.body.substring(0, 100) : roomMessage.body}',
    );
    ...
    if (roomMessage.messageType == 'image' ||
      ...
      log(
        '📎 [GroupChatScreen] Processing file message - messageType: ${roomMessage.messageType}',
      );
      try {
        ...
        if (bodyJson != null) {
          ...
          String? normalizeUrl(String? url, String label) {
            ...
            if (transformed != url) {
              log(
                '🔄 [GroupChatScreen] Transformed $label from localhost: $url -> $transformed',
              );
            }
            return transformed;
          }
          ...
          switch (roomMessage.messageType) {
            ...
            case 'file':
              ...
              final isVideoByExtension = _isVideoFile(
                documentUrl,
                documentName,
              );
              final isAudioByMime =
              ...
              final isAudioByExtension = _isAudioFile(
                documentUrl,
                documentName,
              );
            ...
            case 'audio':
              ...
              audioUrl = normalizeUrl(audioUrl, 'audioUrl');
              log(
                '   Extracted audioUrl: ${audioUrl != null ? (audioUrl.length > 50 ? '${audioUrl.substring(0, 50)}...' : audioUrl) : "null"}',
              );
              break;
            case 'video':
              videoUrl =
              ...
              videoUrl = normalizeUrl(videoUrl, 'videoUrl');
              log(
                '   Extracted videoUrl: ${videoUrl != null ? (videoUrl.length > 50 ? '${videoUrl.substring(0, 50)}...' : videoUrl) : "null"}',
              );
              isDocument = false;
              break;
            default:
        ...
        log(
          '❌ [GroupChatScreen] Failed to parse body as JSON for message ${roomMessage.id}: $e',
        );
        log('   Stack trace: $stackTrace');
        log(
          '   Body that failed to parse: ${roomMessage.body.length > 200 ? roomMessage.body.substring(0, 200) + "..." : roomMessage.body}',
        );
        if (roomMessage.body.trim().startsWith('http://') ||
          ...
          if (roomMessage.messageType == 'image') {
            imageUrl = RoomService.transformLocalhostUrl(
              roomMessage.body.trim(),
            );
          } else if (roomMessage.messageType == 'file') {
            documentUrl = RoomService.transformLocalhostUrl(
              roomMessage.body.trim(),
            );
            isDocument = true;
          } else if (roomMessage.messageType == 'voice' ||
              roomMessage.messageType == 'audio') {
            audioUrl = RoomService.transformLocalhostUrl(
              roomMessage.body.trim(),
            );
          }
        }
      ...
      log(
        'ℹ️ [GroupChatScreen] Not a file message - messageType: ${roomMessage.messageType}',
      );
    }

    log(
      '✅ [GroupChatScreen] Final extracted URLs - imageUrl: ${imageUrl != null ? "present" : "null"}, documentUrl: ${documentUrl != null ? "present" : "null"}, audioUrl: ${audioUrl != null ? "present" : "null"}, videoUrl: ${videoUrl != null ? "present" : "null"}, isVideo: $isVideo',
    );
    ...
    log(
      '📝 [GroupChatScreen] Created GroupMessage: id=${groupMessage.id}, imageUrls=${groupMessage.imageUrls != null ? "present (${groupMessage.imageUrls!.length} items)" : "null"}, documentUrl=${groupMessage.documentUrl != null ? "present (${groupMessage.documentUrl!.length} chars)" : "null"}, isDocument=${groupMessage.isDocument}',
    );
    ...
    Future.delayed(const Duration(milliseconds: 500), () {
      ...
      if (index != -1 &&
        ...
        Future.delayed(const Duration(seconds: 1), () {
          if (mounted) {
            final deliveredIndex = _messages.indexWhere(
              (m) => m.id == messageId,
            );
            if (deliveredIndex != -1) {
              setState(() {
                                ...
                                Builder(
                                  builder: (context) {
                                    ...
                                    if (message.firstImageUrl != null) {
                                      log(
                                        '🖼️ [GroupChatScreen] Rendering image - firstImageUrl: ${message.firstImageUrl!.length > 50 ? message.firstImageUrl!.substring(0, 50) + "..." : message.firstImageUrl}',
                                      );
                                    }
                                    return Stack(
                                      children: [
                                          ...
                                          onTap: () {
                                            if (message.imageFile != null) {
                                              _previewImage(message.imageFile!);
                                                ...
                                                null) {
                                              log(
                                                '👆 [GroupChatScreen] Image tapped - previewing: ${message.firstImageUrl}',
                                              );
                                              _previewImageUrl(
                                                ...
                                                ? Image.file(
                                                    ...
                                                    errorBuilder:
                                                        (
                                                          ...
                                                          log(
                                                            '❌ [GroupChatScreen] Failed to load local image file: $error',
                                                          );
                                                          return Container(
                                                            height: 150,
                                                            color: Colors
  ...
  Widget _buildReactions(GroupMessage message) {
    ...
    if (widget.currentUserId.isNotEmpty) {
      for (final reactionType in uniqueReactionTypes) {
        ...
        if (hasUserReacted) {
          log(
            '🔍 [GroupChatScreen] User has reacted with $reactionType - cancel icon should show',
          );
        }
      }
    }
  ...
  Future<void> _sendVoiceMessage() async {
    ...
    try {
      ...
      if (_recordingFile != null && await _recordingFile!.exists()) {
        ...
        try {
          log(
            '📤 [GroupChatScreen] Uploading voice note to S3 for room: ${widget.group.id}',
          );
          ...
          if (!uploadResponse.success || uploadResponse.data == null) {
            log(
              '❌ [GroupChatScreen] Failed to upload voice note: ${uploadResponse.error}',
            );
            final index = _messages.indexWhere((m) => m.id == messageId);
            if (index != -1) {
              setState(() {
          ...
          if (fileUrl != originalUrl) {
            log(
              '🔄 [GroupChatScreen] Transformed audio fileUrl from localhost: $originalUrl -> $fileUrl',
            );
          }
          ...
          log(
            '📤 [GroupChatScreen] Sending WebSocket voice message with file_url: $fileUrl',
          );
          final sent = await _chatService.sendMessage(
            roomId: widget.group.id,
            content: content,
  ...
  Widget _buildNetworkImage(String imageUrl) {
    ...
    if (normalizedUrl != imageUrl) {
      log(
        '🔄 [GroupChatScreen] Normalized image URL: $imageUrl -> $normalizedUrl',
      );
    }
  ...
  Widget _buildPreviewNetworkImage(String imageUrl) {
    ...
    if (normalizedUrl != imageUrl) {
      log(
        '🔄 [GroupChatScreen] Normalized preview image URL: $imageUrl -> $normalizedUrl',
      );
    }
    ...
    return CachedNetworkImage(
      ...
      errorWidget: (context, url, error) {
        log(
          '❌ [GroupChatScreen] Failed to load preview image: $url, error: $error',
        );
        // Try http if https failed
        if (url.startsWith('https://')) {
          final httpUrl = url.replaceFirst('https://', 'http://');
  ...
  Future<void> _downloadImageFromUrl(String imageUrl) async {
    try {
      ...
      if (imageUrl.contains('localhost') || imageUrl.contains('127.0.0.1')) {
        log(
          '⚠️ [GroupChatScreen] Attempting to download from localhost URL: $imageUrl',
        );
  ...
  Future<void> _downloadDocumentFromUrl(
    ...
    try {
      ...
      if (documentUrl.contains('localhost') ||
        ...
        log(
          '⚠️ [GroupChatScreen] Attempting to download from localhost URL: $documentUrl',
        );
        log('   This may fail on mobile devices if not on same network.');
      }
                            ...
                            if (message.reactions.isEmpty) {
                              ...
                              if (updatedIndex != -1) {
                                ...
                                if (isAlreadyReacted && reactionType != null) {
                                  ...
                                  log(
                                    '🔄 [GroupChatScreen] User updating reaction from ${userReaction.reactionType} to $reactionType',
                                  );
                                  _addReaction(updatedMessage, emoji);
                                } else {
                                  // User hasn't reacted - ADD (POST)
                                  log(
                                    '➕ [GroupChatScreen] User adding new reaction: $reactionType',
                                  );
                                  _addReaction(updatedMessage, emoji);
                                }
                              } else {
  ...
  Future<void> _fetchReactionsForMessagesOnEntry(
    ...
    log(
      '🔄 [GroupChatScreen] Fetching reactions for ${messagesToFetch.length} messages on entry',
    );
      ...
      if (_reactionFetchCooldownUntil != null) {
        ...
        if (now.isBefore(_reactionFetchCooldownUntil!)) {
          final remainingSeconds = _reactionFetchCooldownUntil!
              ...
              .inSeconds;
          log(
            '⏸️ [GroupChatScreen] Reaction fetch on cooldown. ${remainingSeconds}s remaining. Stopping batch fetch.',
          );
          break;
        } else {
    ...
    log(
      '✅ [GroupChatScreen] Completed reaction fetching for messages on entry',
    );
  }
  ...
  Future<void> loadReactionsForMessage(String messageId) async {
    ...
    if (_reactionsFetchInProgress.contains(messageId)) {
      log(
        '⏭️ [GroupChatScreen] Reaction fetch already in progress for message: $messageId',
      );
      return;
    }
    ...
    if (_reactionFetchCooldownUntil != null) {
      ...
      if (now.isBefore(_reactionFetchCooldownUntil!)) {
        final remainingSeconds = _reactionFetchCooldownUntil!
            ...
            .inSeconds;
        log(
          '⏸️ [GroupChatScreen] Reaction fetch on cooldown. ${remainingSeconds}s remaining',
        );
        return;
      } else {
    ...
    if (_lastReactionFetchTime != null) {
      ...
      if (timeSinceLastFetch < _reactionFetchMinInterval) {
        ...
        log(
          '⏸️ [GroupChatScreen] Rate limiting reaction fetch. Waiting ${waitTime.inMilliseconds}ms',
        );
    ...
    try {
      ...
      if (response.statusCode == 429) {
        ...
        log(
          '⚠️ [GroupChatScreen] Rate limit (429) on reaction fetch. Cooldown until: $_reactionFetchCooldownUntil',
        );
      ...
      if (response.success && response.data != null) {
        ...
        if (msgIndex != -1 && mounted) {
          ...
          log(
            '✅ [GroupChatScreen] Reactions updated. Message now has ${updatedMessage.reactions.length} reactions',
          );
          ...
          if (_hasUserReactedAny(updatedMessage)) {
            ...
            final userReactions = updatedReactions
                ...
                .toList();
            log(
              '   User has ${userReactions.length} reaction(s): ${userReactions.map((r) => r.reactionType).join(", ")}',
            );
          }
        } else {
          log(
            '⚠️ [GroupChatScreen] Message not found or not mounted: $messageId',
          );
        }
  ...
  Future<void> _addReaction(GroupMessage message, String reaction) async {
    ...
    if (message.id.isEmpty ||
      ...
      log(
        '⚠️ [GroupChatScreen] Invalid message ID format for reaction: ${message.id}',
      );
      EnhancedToast.error(
        context,
    ...
    if (_reactionsUpdateInProgress.contains(message.id)) {
      log(
        '⏭️ [GroupChatScreen] Reaction update already in progress for message: ${message.id}',
      );
      return;
    }
    ...
    if (_reactionUpdateCooldownUntil != null) {
      ...
      if (now.isBefore(_reactionUpdateCooldownUntil!)) {
        final remainingSeconds = _reactionUpdateCooldownUntil!
            ...
            .inSeconds;
        log(
          '⏸️ [GroupChatScreen] Reaction update on cooldown. ${remainingSeconds}s remaining',
        );
        EnhancedToast.warning(
          context,
          title: 'Too Many Requests',
    ...
    if (_lastReactionUpdateTime != null) {
      ...
      if (timeSinceLastUpdate < _reactionUpdateMinInterval) {
        ...
        log(
          '⏸️ [GroupChatScreen] Rate limiting reaction update. Waiting ${waitTime.inMilliseconds}ms',
        );
        await Future.delayed(waitTime);
      }
    }
    ...
    try {
      ...
      if (hasUserReacted && existingReaction != null) {
        ...
        log(
          '🔄 [GroupChatScreen] Calling PUT API to UPDATE reaction: ${existingReaction.reactionType} → $reactionType (message ${message.id})',
        );
        ...
        log(
          '➕ [GroupChatScreen] Calling POST API to ADD reaction: $reactionType → message ${message.id}',
        );
      ...
      if (response.statusCode == 429) {
        ...
        log(
          '⚠️ [GroupChatScreen] Rate limit (429) on reaction update. Cooldown until: $_reactionUpdateCooldownUntil',
        );
      ...
      log(
        '❌ [GroupChatScreen] Exception ${hasUserReacted ? 'updating' : 'adding'} reaction: $e',
      );
    } finally {
  ...
  Future<void> _removeReaction(
    ...
    if (message.id.isEmpty ||
        (!message.id.contains('-') && int.tryParse(message.id) != null)) {
      log(
        '⚠️ [GroupChatScreen] Invalid message ID format for remove reaction: ${message.id}',
      );
      EnhancedToast.error(
        context,
        title: 'Invalid Message',
    ...
    if (_lastReactionUpdateTime != null) {
      ...
      if (timeSinceLastUpdate < _reactionUpdateMinInterval) {
        final waitTime = _reactionUpdateMinInterval - timeSinceLastUpdate;
        log(
          '⏸️ [GroupChatScreen] Rate limiting reaction delete. Waiting ${waitTime.inMilliseconds}ms',
        );
        await Future.delayed(waitTime);
      }
    }
    ...
    try {
      ...
      if (response.statusCode == 429) {
        ...
        log(
          '⚠️ [GroupChatScreen] Rate limit (429) on reaction delete. Cooldown until: $_reactionUpdateCooldownUntil',
        );
                          ...
                          onPressed: isSaving
                              ...
                              : () async {
                                  ...
                                  try {
                                    ...
                                    log(
                                      '📝 [GroupChatScreen] Editing message: ${message.id}',
                                    );
                                    ...
                                    if (response.success &&
                                        response.data != null) {
                                      log(
                                        '✅ [GroupChatScreen] Message edited successfully',
                                      );
                                    ...
                                    log(
                                      '❌ [GroupChatScreen] Error editing message: $e',
                                    );
                                    if (!mounted) return;
  ...
  Future<void> _sendImageMessageWithProgress(File imageFile) async {
    ...
    try {
      ...
      if (!uploadResponse.success || uploadResponse.data == null) {
        log(
          '❌ [GroupChatScreen] Failed to upload image to S3: ${uploadResponse.error}',
        );
      ...
      log(
        '📤 [GroupChatScreen] Sending WebSocket message with file_url: $fileUrl',
      );
      final sent = await _chatService.sendMessage(
        roomId: roomId,
        content: content,
  ...
  Future<void> _sendVideoMessageWithProgress(File videoFile) async {
    ...
    try {
      ...
      if (!uploadResponse.success || uploadResponse.data == null) {
        log(
          '❌ [GroupChatScreen] Failed to upload video: ${uploadResponse.error}',
        );
        final index = _messages.indexWhere((m) => m.id == messageId);
        if (index != -1) {
          setState(() {
      ...
      if (fileUrl != originalUrl) {
        log(
          '🔄 [GroupChatScreen] Transformed video fileUrl from localhost: $originalUrl -> $fileUrl',
        );
      }
      ...
      log(
        '📤 [GroupChatScreen] Sending WebSocket video message with file_url: $fileUrl',
      );
      final sent = await _chatService.sendMessage(
        roomId: roomId,
        content: content,
  ...
  Future<void> _sendDocumentMessageWithProgress(
    ...
    try {
      ...
      if (!uploadResponse.success || uploadResponse.data == null) {
        log(
          '❌ [GroupChatScreen] Failed to upload document to S3: ${uploadResponse.error}',
        );
        // Update message to show error
        final index = _messages.indexWhere((m) => m.id == messageId);
        if (index != -1) {
    ...
    try {
      ...
      if (response.success) {
        ...
        if (_isGroupInfoPageOpen && mounted) {
          ...
          final updatedMembers = previousMembers
          ...
          if (removedCount > 0) {
            ...
            if (_groupInfoMemberCountNotifier != null) {
              ...
              _groupInfoMemberCountNotifier!.value = newCount < 0
                  ? 0
                  : newCount;
            }
          }
        ...
        if (_isGroupInfoPageOpen && mounted) {
          await _refreshGroupInfoPage(updateInPlace: true, forceNetwork: true);
        }
  ...
  GroupMessage? _findMessageById(
    String? targetId,
    Map<String, GroupMessage> messageMap,
      ...
      for (
        ...
        if (response.success && response.data != null) {
          ...
          try {
            ...
            final repliedToGroupMessage = _convertRoomMessageToGroupMessage(
              repliedToRoomMessage,
            );
            ...
            if (mounted) {
              setState(() {
                ...
                final repliedToIndex = _messages.indexWhere(
                  (m) => m.id == repliedToGroupMessage.id,
                );
                if (repliedToIndex == -1) {
                  ...
                  for (int i = 0; i < _messages.length; i++) {
                    if (_messages[i].timestamp.isAfter(
                      repliedToGroupMessage.timestamp,
                    )) {
                      insertIndex = i;
                      break;
                    }
      ...
      PageRouteBuilder(
        pageBuilder: (context, animation, secondaryAnimation) =>
            CreateGroupPage(
              ...
              isAddMemberMode:
                  true, // Set flag to disable group information card
            ),
    ...
    try {
      ...
      final newMembers = response.members
          .map((memberData) {
            ...
            // Extract unit
            final unit =
                memberData['unit_flat_number']?.toString() ??
            ...
            // Extract building
            final building =
                memberData['soc_building_name']?.toString() ??
            ...
            // Extract phone
            final phone =
                memberData['member_mobile_number']?.toString() ??
                      ...
                      : ListView.builder(
                          ...
                          itemBuilder: (context, index) {
                            ...
                            return ListTile(
                              ...
                              trailing: isDisabled
                                  ? ElevatedButton.icon(
                                      onPressed: () => OneAppShare.shareInvite(
                                        name: member.name,
                                      ),
                                      icon: const Icon(
                                        Icons.person_add,
