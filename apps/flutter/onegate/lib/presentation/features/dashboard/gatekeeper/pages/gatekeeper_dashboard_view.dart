...
class _QRScannerScreenState extends State<QRScannerScreen>
  ...
  // Verify passcode method (simulated)
  Future<bool> _verifyPasscode(String passcode) async {
    ...
    try {
      ...
      // ✅ Await API call and store response as a Map
      Map<String, dynamic> response = await remoteDataSource.verifyPasscode(
        companyId: "",
        passcode: passcode,
      );

      // ✅ Extract and return boolean result
      return response['status'] == 'valid'; // Returns true if status is "valid"
    } catch (e) {
  ...
  void _onQRViewCreated(QRViewController controller) {
    ...
    controller.scannedDataStream.listen((scanData) async {
      if (!mounted || _isProcessing || scanData.code == null) return;
      ...
      bool isValid = await _verifyPasscode(scanData.code!);

      if (!mounted) return;

      if (isValid) {
        _showSuccessDialog(scanData.code!);
      } else {
        _showErrorDialog();
      }
  ...
  void _showSuccessDialog(String code) {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text('QR Code verified: $code'),
        actions: [
          TextButton(
            onPressed: () {
